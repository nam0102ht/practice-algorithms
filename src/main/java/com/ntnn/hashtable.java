...
class Result {
    ...
    public static void checkMagazine(List<String> magazine, List<String> note) {
        ...
        for(String i : magazine){
            if(!magazineOccurences.containsKey(i)) {
                magazineOccurences.put(i, 0);
            }
            magazineOccurences.put(i, magazineOccurences.get(i) + 1);
        }
        for(String i : note){
            if(!ransomOccurences.containsKey(i)) {
                ransomOccurences.put(i, 0);
            }
            ransomOccurences.put(i, ransomOccurences.get(i) + 1);
        }
